...
def create_contract_analysis_pdf(data: Dict[str, Any], lang="中文"):
    ...
    LABEL_MAP = {
        "中文": {
            ...
            "RiskTitle": "【 综合风险观察 】",
            "RiskDefault": "无显著高危条款",
            "Disclaimer": "本报告由 Hao Harbour AI 自动生成，旨在协助合同阅读。Hao Harbour 不承担任何法律担保责任。请务必结合合同原文并咨询法律专家。"
        },
        "English": {
            "Title": "HAO HARBOUR - Deep Dive Contract Intelligence Report",
            "MetaTitle": "[ Core Financials & Key Terms Overview ]",
            "RiskTitle": "[ Comprehensive Risk Observation ]",
            "RiskDefault": "No critical risks identified",
            "Disclaimer": "This report is generated by Hao Harbour AI for assistance only. Hao Harbour assumes no legal liability. Please consult a professional."
        }
    }
    ...
    try:
        ...
        f_section = ImageFont.truetype("simhei.ttf", 48)
    ...
    # --- 核心算法：按单词换行 (Word-wrap)，防止单词被劈开 ---
    def get_lines_v2(text, font, max_width):
    ...
    # --- 核心算法：将文本渲染为 Bullet Points 列表 ---
    def draw_smart_content(draw, text, x, y, font, max_width, fill=(40, 40, 40)):
        # 识别 AI 返回的 point：按序号(1. 2.)、分号或换行符拆分
        ...
        for p in points:
            ...
            # 绘制文字，带缩进
            lines = get_lines_v2(p, font, max_width - BULLET_INDENT)
            for line in lines:
    ...
    # 1. 页眉 Banner (深色全宽)
    draw.rectangle([(0, 0), (CANVAS_W, 240)], fill=(30, 30, 30))
    draw.text((SIDE_MARGIN, 85), L["Title"], font=f_banner, fill=(210, 180, 120))
    ...
    for key, label in meta_list:
        ...
        # Metadata 也需处理换行
        lines = get_lines_v2(val, f_body, MAX_TEXT_W - LABEL_COL_W)
        for line in lines:
            draw.text((SIDE_MARGIN + LABEL_COL_W, y), line, font=f_body, fill=(30, 30, 30))
            y += f_body.size + 15
        y += 45 # 每行元数据后的间距
    ...
    for s in data.get('Sections', []):
        heading = sanitize_text(s.get('Heading', 'Section Summary'))
        # 左侧装饰条
        draw.rectangle([(SIDE_MARGIN, y), (SIDE_MARGIN + 15, y + 55)], fill=(191, 160, 100))
        draw.text((SIDE_MARGIN + 45, y), heading, font=f_section, fill=(40, 40, 40))
        y += 100
        # 智能渲染为列表
        y = draw_smart_content(draw, s.get('Content', ''), SIDE_MARGIN + 45, y, f_body, MAX_TEXT_W - 45)
        y += 80

    y += 40
    # 4. 综合风险观察 (Risks)
    draw.rectangle([(SIDE_MARGIN, y), (CANVAS_W - SIDE_MARGIN, y + 8)], fill=(220, 50, 50))
    y += 60
    draw.text((SIDE_MARGIN, y), L["RiskTitle"], font=f_section, fill=(220, 50, 50))
    y += 110
    risks_txt = data.get('Risks', L["RiskDefault"])
    y = draw_smart_content(draw, risks_txt, SIDE_MARGIN + 45, y, f_body, MAX_TEXT_W - 45, fill=(180, 0, 0))

    # 5. 水印铺设 (铺满当前内容区域)
    wm_layer = Image.new('RGBA', canvas.size, (0, 0, 0, 0))
    wm_draw = ImageDraw.Draw(wm_layer)
    for row in range(0, y + 1000, 1200):
        for col in range(0, CANVAS_W, 800):
            wm_draw.text((col, row), "HAO HARBOUR", font=f_wm, fill=(180, 180, 180, 45))
    ...
    footer_h = 350
    content_end_y = y + 100
    final_canvas = canvas.crop((0, 0, CANVAS_W, int(content_end_y + footer_h)))
    f_draw = ImageDraw.Draw(final_canvas)
    
    # 页脚灰色背景
    f_draw.rectangle([(0, content_end_y), (CANVAS_W, content_end_y + footer_h)], fill=(245, 245, 245))
    # 自动换行绘制声明文字
    draw_wrapped_text_footer = get_lines_v2(L["Disclaimer"], f_footer, MAX_TEXT_W)
    fy = content_end_y + 100
    for fline in draw_wrapped_text_footer:
        f_draw.text((SIDE_MARGIN, fy), fline, font=f_footer, fill=(150, 150, 150))
        fy += f_footer.size + 15
